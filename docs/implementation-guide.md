...
- discussion writes now flow through local API routes into a local SQLite-backed packet store with cryptographic person-actor provenance, and the active discussion model now writes canonical `Discussion(kind: "space" | "forum" | "topic" | "message")` packets while still projecting legacy `DiscussionSpace -> DiscussionForum -> DiscussionThread -> DiscussionPost -> DiscussionReply` views where needed for compatibility
...
### 2026-04-25 - Packet compatibility coverage is now classified explicitly, and discussion-family cleanup is deferred behind adapters

- Context: the fortress/auth milestone is now stable enough that the next architectural risk is less about write admission and more about whether packet compatibility and discussion-family semantics are honest enough to support Packet Explorer, import/export, and validation without another schema-churn surprise.
- Options considered: keep compatibility support implicit, start renaming discussion packets immediately into a new super-family, or land a narrow audit pass that classifies compatibility support explicitly and records the discussion migration boundary without changing stored packet families yet.
- Decision: packet-family compatibility summaries now classify each family explicitly, expose supported schema versions plus adapted-write preparation capability, and keep discussion-family evolution on an adapter-backed path instead of a silent rename. That audit entry is now historical rather than final: `Element`, `Claim`, and `Policy` still use same-family schema adapters, while `Discussion` now exists as the canonical family for new writes and the legacy discussion families remain readable/projectable through the family-evolution bridge.
- Why: Packet Explorer needs truthful answers about what can be inspected, adapted, and safely rewritten, and the repo needs an explicit guardrail so future cleanup does not smuggle in a risky discussion-family rewrite under the label of "just tidying."
- Consequences / follow-ups: the compatibility layer now emits machine-readable audit summaries for future raw/adapted packet inspection, and the remaining work is no longer "whether discussion unifies" but "how much of that family evolution gets pulled into the shared builder/interpreter pipelines before Packet Explorer." Core should continue to own candidate planning, policy/digest law, and compatibility-aware packet shaping, while runtime keeps session proof plumbing, ticket persistence, database writes, and derived-state refreshes.

### 2026-04-25 - Auth resume and target-version adapters stabilize the fortress milestone

- Context: locked claimed-bundle writes could decrypt successfully but immediately retry the queued action before React had committed the unlocked identity, causing a second unlock popup and leaving the first action uncompleted even though the identity was now unlocked.
- Options considered: delay action resume with timers, special-case attestation writes, or carry the freshly decrypted identity through a synchronous handoff until normal identity state catches up.
...
### 2026-04-28 - Packet pipeline consolidation starts moving build and interpretation out of one-off seams

- Context: the shared mutation corridor, canonical `Discussion` family, and target-version schema adapters were in place, but packet construction still lived in family-specific helpers and discussion family evolution still depended on a bespoke bridge that Packet Explorer would otherwise have to special-case.
- Options considered: leave the existing helpers alone until Packet Explorer, rewrite all families into a new generic system at once, or add one reusable pipeline and prove it first on the already-active `Discussion` family plus one simpler companion family.
- Decision: the repo now has an additive generic packet builder pipeline plus family-owned build modules, with `Discussion` and `Policy` as the first migrated families. It also has a unified interpreter shell that wraps the existing schema-version compatibility layer for every family and plugs discussion family-evolution into the same contract for canonical, legacy, and UI-targeted views.
- Why: Packet Explorer and later family migrations need one place to ask for packet interpretation and one place to build canonical packets, otherwise every new family rewrite becomes a new round of helper drift.
- Consequences / follow-ups: discussion canonicalization now uses a central canonical-target resolver so repeated legacy touches reuse the same deterministic mirror id and prefer the current canonical revision when it exists; route/service audits now classify direct packet writers explicitly and block known mutating service calls from non-canonical API routes; and the packet-family pipeline inventory now records which families are on the generic builder, the unified interpreter, or neither so future cleanup can expand the pattern deliberately instead of by folklore.

### 2026-04-29 - Raw packet signatures verify before adaptation, and migrations must carry raw-compat rules

- Context: claimed-session auth failures persisted even after session recovery work because older signed packets were still being reparsed into the adapted current schema before cryptographic verification, which injected additive defaults such as `header.metadata.compatibility`, `body.claimed_role_refs`, and `body.locality` that were not present in the original signed bytes.
- Options considered: keep expanding auth/session fallbacks, rewrite stored packets in place, or move signature verification fully onto the raw stored/request envelope while preserving adapted packets as the runtime read view.
- Decision: cryptographic verification now starts from the raw envelope, generates raw compatibility candidates that preserve omission-vs-null history, and only parses the adapted packet after the signature is known-good. Auth failure classification now distinguishes canonicalization mismatch from true signature failure, schema failure, and metadata failure. Actor-candidate resolution now dedupes on raw unsigned packet identity rather than revision id alone so a corrupted stored `@rN` copy cannot hide a valid request/session `@rN` packet for the same actor.
