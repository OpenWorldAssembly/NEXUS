...
- Decision: packet-family compatibility summaries now classify each family as either `current_only` or `legacy_supported`, expose supported schema versions plus adapted-write preparation capability, and test that `Element`, `Claim`, and `Policy` remain the only explicitly legacy-supported families today while discussion packets are intentionally current-schema-only. `DiscussionSpace`, `DiscussionForum`, `DiscussionThread`, `DiscussionPost`, and `DiscussionReply` remain the current stored families; any future unification into one conceptual discussion domain must happen as an adapter-backed migration project rather than a quick rename.
...
- Consequences / follow-ups: the compatibility layer now emits machine-readable audit summaries for future raw/adapted packet inspection; discussion-family semantics are explicitly frozen for now; and the next low-risk core-authority extraction order should be `actor.write_policy.update`, claim-style mutations, locality/home-locality mutations, and discussion-surface ensure before revisiting discussion thread/reply planning itself. For those extractions, core should own pure candidate planning, policy/digest law, and compatibility-aware packet shaping, while runtime should keep session proof plumbing, ticket persistence, database writes, and derived-state refreshes.
