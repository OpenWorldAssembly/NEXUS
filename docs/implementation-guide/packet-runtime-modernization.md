...
`npm run audit:trusted-coordinators` checks the scaffold manifest. The audit currently treats Dispatch, Request, Definition, Regulation, Planning, Building, Inspection, Certification, Archive, Verification, and Projection as foldered gated coordinators. Resolution remains legacy-flat with a warning until it is promoted.
