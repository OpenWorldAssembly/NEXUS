...
| Text inputs, composers, search fields, and form rows | `ui/forms/search/*`; raw `TextInput` in `trust`, `roles`, `discussions`, packet explorer import/export notes; `nexus-identity-ui.tsx` identity fields; discussion reply/post composers | Search field/list chrome is shared; broader form shells remain mixed | Search result dropdowns in Explorer, identity, and locality create now use shared search primitives and result loading boundaries. Multiline composers, passphrase fields, note fields, and general label/error/hint patterns still need a universal field/composer shell | `ui/forms` with field shell, text input, textarea/composer, search box, result list, error text, hint text, and field action row | High |
...
- `app/components/nexus/ui/forms`
  - field shell, text input, textarea/composer, `search/*`, picker shell, toggles
