...
- scopes with no local discussion forums show an explicit empty state rather than a persistent loading message, and eligible home-branch geographic scopes can backfill the empty OWA discussion bundle through a signed `PUT /api/nexus/scopes/[scopeId]/discussions/surfaces` action
...
- discussion thread/post/reply creation is now intent-first: the browser submits forum/thread/reply intent plus actor assertion fields, runtime and core re-derive the canonical candidate packets, and any signed packet bundle included in the request is verified only as a signer artifact that must exactly match the canonical candidate before it can be persisted
- the first shared fortress mutation corridor is now active for migrated discussion and attestation writes: `/api/nexus/mutations/prepare` returns canonical unsigned packet candidates plus one-time tickets, the current web identity shell signs those prepared packets locally, and `/api/nexus/mutations/finalize` re-checks digest match, signature validity, proof level, authority, and policy before runtime writes any packet effects
- prepared mutation proof requirements are now enforced generically at finalize: runtime reads the stored ticket before auth verification, consumes any needed reauth because the prepared mutation required it, and the corridor rejects signed packet bundles whose proof bundle does not satisfy the ticket's `required_proof_level` plus its accepted proof-method set
...
- the first fortress write-policy slice is now active for discussions: scope `Element` packets may reference typed `Policy(kind: "write_lock")` packets through `header.moderation.policy_refs`, and those policies can raise the required proof level for `discussion.thread.create`, `discussion.post.create`, and `discussion.reply.create`
- actor write approval remains packet-backed and is now governed by the current effective policy: changing the actor `write_lock` policy is evaluated under the actor's active effective mode, `standard` allows ordinary packet-backed updates, guarded or stronger modes require fresh approval for `actor.write_policy.update`, and create/claim still bootstrap a first explicit policy without recreating duplicate deterministic revisions
- discussion controls are not disabled purely because a claimed local bundle is locked; signing readiness is enforced by the shared verified-write layer, while the route surfaces the shared auth-gate modal for sign-in, unlock, write approval, or community-claim guidance
- the shared auth-gate surface can now also be spawned as a standalone local-bundle unlock flow from account and identity-security surfaces, and protected writes now use the same password-first popup with focused passphrase entry, Enter-to-submit, pending-action resume, and `Use passkey` / `Set up passkey` secondary options where appropriate
...
- top-level thread creation is written through `/api/nexus/scopes/[scopeId]/discussions/posts` using actor assertion fields plus canonical intent (`forum_packet_id`, `thread_title`, `post_markdown`, `created_at`, `mutation_nonce`, optional thread metadata) and a matching signed prepared-candidate bundle; reply creation is written through `/api/nexus/scopes/[scopeId]/discussions/replies` using actor assertion fields plus canonical intent (`parent_post_packet_id`, `reply_markdown`, `created_at`, `mutation_nonce`) and a matching signed prepared-candidate bundle; packet-signal vote reactions now also run through the shared prepare/finalize corridor instead of trusting a client-authored attestation packet as the authoritative payload
...
- guests can see protected role actions, but pressing them opens the shared sign-in-required auth-gate modal instead of attempting the write or surfacing raw backend identity errors
...
- the active write-approval update flow is now packet-backed: changing the setting prepares and signs a typed actor `Policy(kind: "write_lock")` packet revision and, when needed, a matching actor `Element` policy-ref revision through the shared mutation corridor; the legacy auth-store `security_mode` row remains a fallback compatibility source rather than the primary write-truth
- when fresh proof is required for a write-approval change, the same shared auth popup captures either a fresh bundle-passphrase proof or passkey confirmation and then resumes the pending change without discarding local UI state
