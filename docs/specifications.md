...
- top-level thread creation, reply creation, discussion-surface ensure, and packet-signal vote reactions now use the shared `/api/nexus/mutations/prepare` plus `/api/nexus/mutations/finalize` corridor; the older route-specific write endpoints return deprecation responses rather than acting as alternate write corridors
