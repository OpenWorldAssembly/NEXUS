...
Runtime server code is now being organized around bounded Nexus contexts under `runtime/nexus/server/*`: identity, discussion, reaction, locality, scope, Packet Explorer, fortress, readiness, and shared helpers. Compatibility bridge files preserve older top-level imports during migration, but new runtime implementation work should land in the bounded-context folders unless it belongs in `core/*`, `runtime/storage/*`, or `runtime/trusted_coordinators/*`.
