...
export const FORTRESS_HANDLER_GENERICIZATION_ENTRIES = [
  ...
  },
  {
    ...
    next_step:
      'Enroll packet-signal attestation as a manifest-backed Attestation planner once target summary lookup is isolated.',
    notes:
      'Single Attestation write with a small target-summary read dependency.',
  },
  {
    ...
    genericization_status: 'workflow_specific',
    next_step:
      'Separate assembly element creation from optional default discussions and optional association claim workflow.',
    notes:
  ...
  },
  {
    ...
    genericization_status: 'planner_extraction_needed',
    next_step:
      'Promote scoped relation planner outputs into generic Relation/Claim write plans.',
    notes:
      'Relation-first association write still emits compatibility claim state for projection support.',
  },
  {
    mutation_intent: 'assembly_association.relation.clear',
    domain: 'relation',
    prepare_handler: 'prepareAssemblyAssociationRelation',
    finalize_handler: 'finalizeAssociationRelationUpdate',
    packet_types_touched: ['Relation', 'Claim'],
  ...
  {
    mutation_intent: 'home_locality.relation.set',
    domain: 'relation',
  ...
  {
    mutation_intent: 'follows.relation.clear',
    domain: 'relation',
    prepare_handler: 'prepareFollowRelation',
    finalize_handler: 'finalizeFollowRelationUpdate',
    packet_types_touched: ['Relation'],
  ...
  },
  {
    ...
    genericization_status: 'generic_ready',
    next_step:
      'Enroll role association claim as a generic Claim revision after role scope resolution is isolated.',
    notes: 'Single Claim write with Role authority-scope lookup.',
  },
  {
    mutation_intent: 'role_association.attestation.set',
    domain: 'role',
    ...
    operation_kinds: ['attestation.set', 'attestation.clear', 'workflow.compose'],
    operation_mapping_status: 'runtime_workflow_gap',
    genericization_status: 'workflow_specific',
    next_step:
      'Extract support/dispute mutual-exclusion planning before generic Attestation enrollment.',
    notes:
      'Can emit multiple Attestation revisions to clear opposing state and compute projection counts.',
  },
  {
