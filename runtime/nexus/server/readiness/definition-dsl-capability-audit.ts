...
export function createDefinitionDslCapabilityAuditReport(): DefinitionDslCapabilityAuditReport {
  ...
  const discussionDefaultRecipeIsDefinitionSeeded = hasAll(discussionDefinition, [
    'discussion.defaults_definition.element_surface_recipe.v0',
  ...
  const entries: DefinitionDslCapabilityEntry[] = [
    ...
    createEntry({
      area: 'seed_defaults',
      layer: 'adapter_runtime',
      status: discussionDefaultRecipeIsDefinitionSeeded
        ? 'definition_ready'
        : hasAll(discussionDefaults, [
      ...
      evidence: discussionDefaultRecipeIsDefinitionSeeded
        ? [
          'Default discussion surface recipes are carried by the Discussion defaults_definition seed material.',
          'Helper code remains the trusted local executor, while Definition metadata carries the reseed recipe identity, profiles, deterministic id strategy, and builder name.',
        ]
        : [
      ...
      next_step:
        discussionDefaultRecipeIsDefinitionSeeded
          ? 'Use the seeded defaults_definition recipe during reseed fixture construction; keep helper execution trusted-local.'
  ...
  findings.push({
    severity: discussionDefaultRecipeIsDefinitionSeeded ? 'info' : 'warning',
    area: 'seed_defaults',
    code: discussionDefaultRecipeIsDefinitionSeeded
      ? 'discussion_defaults_definition_seeded'
      : 'discussion_defaults_not_definition_seeded',
    message: discussionDefaultRecipeIsDefinitionSeeded
      ? 'Default discussion surface recipes are represented in Definition/default seed material; trusted helper code remains the executor.'
