...
import {
  createPreferenceScopeDisplayManifestShadowFortressPlan,
...
test('resolves a Preference.scope_display manifest action into a shadow fortress plan', () => {
  const plan = createPreferenceScopeDisplayManifestShadowFortressPlan({
  ...
  assert.equal(plan.bridge_kind, 'preference.scope_display.shadow_fortress_prepare');
  ...
  assert.equal(plan.action_plan.planner_id, 'preference.scope_display.latest_active_revision.v0');
  assert.deepEqual(plan.action_plan.builder_ids, ['preference.scope_display.body.v0']);
  assert.deepEqual(plan.action_plan.policy_action_ids, ['preference.scope_display.write']);
  assert.equal(plan.packet_candidate.current_packet_envelope_supported, false);
...
test('keeps Preference manifest mutation out of the live fortress intent registry', () => {
  ...
  assert.equal(liveKinds.includes('preference.scope_display.set'), false);
  assert.equal(liveKinds.includes('preference.scope_display.withdraw'), false);
});
