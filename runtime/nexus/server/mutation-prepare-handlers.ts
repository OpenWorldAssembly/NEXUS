/**
 * File: mutation-prepare-handlers.ts
  ...
  private createPrepareHandlerMap(): Record<MutationPrepareHandlerKey, PrepareHandler> {
    return {
      ...
      preparePacketSignal: async ({ intent, actorPacket, actorKey }) =>
        this.preparePacketSignal({
          intent: intent as Extract<MutationIntent, { kind: 'attestation.packet_signal.set' }>,
          actorPacket,
          actorKey,
        }),
      prepareAssemblyElementCreate: async ({ intent, actorPacket }) =>
        this.prepareAssemblyElementCreate({
          intent: intent as Extract<MutationIntent, { kind: 'assembly.element.create' }>,
          actorPacket,
        }),
      prepareAssemblyAssociationClaimCompatibilityAlias: async ({ intent, actorPacket }) =>
        this.prepareAssemblyAssociationClaimCompatibilityAlias({
          intent: intent as Extract<MutationIntent, { kind: 'assembly_association.claim.set' }>,
          actorPacket,
        }),
      prepareAssemblyAssociationRelation: async ({ intent, actorPacket }) =>
        this.prepareAssemblyAssociationRelation({
          intent: intent as Extract<
            MutationIntent,
            { kind: 'assembly_association.relation.set' | 'assembly_association.relation.clear' }
          >,
          actorPacket,
        }),
      prepareHomeLocalityRelation: async ({ intent, actorPacket }) =>
        this.prepareHomeLocalityRelation({
          intent: intent as Extract<MutationIntent, { kind: 'home_locality.relation.set' }>,
          actorPacket,
        }),
      prepareHomeLocalityClaimCompatibilityAlias: async ({ intent, actorPacket }) =>
        this.prepareHomeLocalityClaimCompatibilityAlias({
          intent: intent as Extract<MutationIntent, { kind: 'home_locality.claim.set' }>,
          actorPacket,
        }),
      prepareFollowRelation: async ({ intent, actorPacket }) =>
        this.prepareFollowRelation({
          intent: intent as Extract<MutationIntent, { kind: 'follows.relation.set' | 'follows.relation.clear' }>,
          actorPacket,
        }),
      prepareRoleAssociationClaim: async ({ intent, actorPacket }) =>
        this.prepareRoleAssociationClaim({
          intent: intent as Extract<MutationIntent, { kind: 'role_association.claim.set' }>,
          actorPacket,
        }),
      prepareRoleAssociationAttestation: async ({ intent, actorPacket }) =>
        this.prepareRoleAssociationAttestation({
          intent: intent as Extract<MutationIntent, { kind: 'role_association.attestation.set' }>,
          actorPacket,
        }),
      prepareDiscussionSurfacesEnsure: async ({ intent, actorPacket }) =>
        this.prepareDiscussionSurfacesEnsure({
          intent: intent as Extract<MutationIntent, { kind: 'discussion.surfaces.ensure' }>,
          actorPacket,
        }),
      prepareActorWritePolicyUpdate: async ({ intent, actorPacket }) =>
        this.prepareActorWritePolicyUpdate({
          intent: intent as Extract<MutationIntent, { kind: 'actor.write_policy.update' }>,
          actorPacket,
        }),
    };
  }
